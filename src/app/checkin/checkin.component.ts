    ...
    this.serve.fetchData({
      ...
      .subscribe(((result: any) => {
        ...
        for (let i = 0; i < this.checkins.length; i++) {
          ...
          this.excel_data.push({ 'Date': this.checkins[i].activity_date, 'Sales User': this.checkins[i].exec_name, 'Type': this.checkins[i].dr_type, 'Company Name': this.checkins[i].other_name == '' ? this.checkins[i].company_name : this.checkins[i].other_name, 'Employee Id': this.checkins[i].employee_id, 'Team Name': this.checkins[i].team_name, 'Team Code': this.checkins[i].team_code, 'Team State': this.checkins[i].team_state, 'Check In': this.checkins[i].visit_start, 'Check Out': this.checkins[i].visit_end, 'Remark': this.checkins[i].description, });
