...
update_distribution() {
  ...
  this.serve.fetchData({ 'id': this.data.id,'type':this.data.change_type,'login_id':this.userId}, "Lead/update_type").subscribe((result => {
    console.log(result);
...
update_basic_address(data)
  {
    ...
    if(this.data.doa > this.data.dob){
    this.serve.fetchData({data,'uid':this.userId,'uname':this.userName},"Distributors/distributors_address_update").subscribe((result=>{
      this.dialog2.closeAll();
    }))
  }
  else{
